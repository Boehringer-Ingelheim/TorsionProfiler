...
"""Gaussian Torsion Profiler:
    this is the command for calculating an Gaussian - wB97XD Torsion Profile
"""
...
from torsion_profiler.engines import GaussianCalculator
...
@click.command(
    "tp-g16",
    short_help="Run a torsionProfile with Gaussian 16 wB97XD",
)
...
@OUTPUT_DIR.parameter(
    help=OUTPUT_DIR.kwargs["help"] + " Defaults to `./torsion_profile_wB97XD_gauss`.",
    default="torsion_profile_wB97XD_gauss",
)
@NMEASUREMENT.parameter()
@SUBMIT.parameter()
def gaus_torsion_profile(
    mol: str,
    ...
    """
    This function is here to implement the Gaussian - torsion profiler
    """
    if submit_cluster:
    ...
    return _base_torsion_profile(
        ...
        _conda_env=conf["conda_calculator_envs"]["GaussianCalculator"],
    )


PLUGIN = TorsionProfilerCommandPlugin(
    command=gaus_torsion_profile,
