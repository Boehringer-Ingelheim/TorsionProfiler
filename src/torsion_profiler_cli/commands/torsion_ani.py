...
"""ANI Torsion Profiler:
    this is the command for calculating an ANI Torsion Profile
"""
...
@click.command(
    "tp-ani",
    short_help="Run a torsionProfile with ANI2x",
)
...
@OUTPUT_DIR.parameter(
    help=OUTPUT_DIR.kwargs["help"] + " Defaults to `./torsion_profile_ani`.",
    default="torsion_profile_ani",
)
@NMEASUREMENT.parameter()
@SUBMIT.parameter()
def ani_torsion_profile(
    mol: str,
    ...
    return _base_torsion_profile(
        ...
        _conda_env=conf["conda_calculator_envs"][AniCalculator.__name__],
    )


PLUGIN = TorsionProfilerCommandPlugin(
    command=ani_torsion_profile,
